...
const FormRegister = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${end}/getRoleById/${id}`, {
        headers: {
  ...
  const handleFormSubmit = async (values, { setSubmitting }) => {
    try {
      const end = process.env.REACT_APP_API_URL;
      const response = await axios.post(`${end}/updateRole/${id}`, values, {
        headers: {
      ...
      if (response.status === 200) {
        // setSubmitting(false);
        toast.success("Update Role Successfuly !");
      ...
      if (error.response.data.status === 400) {
        toast.error("Role is already exist");
      }
    } finally {
      setSubmitting(false);
    }
